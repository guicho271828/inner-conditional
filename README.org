...
In this sample, =body= is meant for output some string to a predefined
stream which is dynamically bound but =*output-stream*= is
bound to nil by default. This means the library allows the user to
output the result as a string by default. Similar technique is used
in =cl-markup= , for example. (I was inspired by the introduction of
that library.)

However in some case a user should run a loop with that macro. The
author may want to divide that conditional part of the code, but
until now there is no standardized way to do that. Now I show you
another version using this library.
...
is checked only once. Below is the sequence of iteratively macroexpanded codes.
